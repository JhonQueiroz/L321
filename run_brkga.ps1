param(
    [string]$BaseDir = "BASE",
    [string]$RootOutputDir = "RESULTS\BRKGA",
    [string]$Executable = "BRKGA/main.jl",
    [string]$ProjectDir = "BRKGA",
    [string]$ConfigFile = "BRKGA/config.conf",
...
if (-not (Test-Path -LiteralPath $ProjectDir -PathType Container)) {
    throw "[ERRO] Pasta de projeto '$ProjectDir' nao encontrada."
}

if (-not (Test-Path -LiteralPath $ConfigFile -PathType Leaf)) {
    throw "[ERRO] Arquivo de configuracao '$ConfigFile' nao encontrado."
}
...
Write-Info "PROJECT_DIR    = $ProjectDir"
Write-Info "CONFIG_FILE    = $ConfigFile"
...
Write-Info "Params: seed=$Seed generations=$Generations"
Write-Host ""
...
$configFullPath = (Resolve-Path -LiteralPath $ConfigFile).Path
...
foreach ($item in $instances) {
    ...
    if (Test-Path -LiteralPath $outCsv -PathType Leaf) {
        Write-Skip "$relativePath (saida ja existe)"
        $skipped++
        continue
    }
    ...
    & $juliaExe --project=$ProjectDir $Executable `
        $configFullPath `
        $instanceFullPath `
        $Seed `
        $Generations `
